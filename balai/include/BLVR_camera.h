...
public:
    ...
    bool GetControllerState(ControllerState& current, ControllerState* prev=NULL) const {
        ...
        ControllerState ans = current;
		float value = ans.Touchpad.x + ans.Touchpad.x + ans.Trigger + ans.Menu + ans.Grip + ans.OnTouchpad + ans.Reserved;
        if (value >= 1.0) {
            BL_LOG("****** GetControllerState: ******\n");
            BL_LOG("<current> \n");
            BL_LOG("Touchpad: %.2f, %.2f\n", ans.Touchpad.x, ans.Touchpad.y);
            BL_LOG("Trigger: %.2f\n", ans.Trigger);
            BL_LOG("Menu: %d\n", ans.Menu);
            BL_LOG("Grip: %d\n", ans.Grip);
            BL_LOG("OnTouchpad: %d\n", ans.OnTouchpad);
            ...
            if (prev) {
                ans = *prev;
                BL_LOG("<pre> \n");
                BL_LOG("<pre> Touchpad: %.2f, %.2f\n", ans.Touchpad.x, ans.Touchpad.y);
                BL_LOG("<pre> Trigger: %.2f\n", ans.Trigger);
                BL_LOG("<pre> Menu: %d\n", ans.Menu);
                BL_LOG("<pre> Grip: %d\n", ans.Grip);
                BL_LOG("<pre> OnTouchpad: %d\n", ans.OnTouchpad);
                BL_LOG("<pre> Reserved: %d\n", ans.Reserved);
            }
            BL_LOG("*********************************\n");
        }
    ...
    int GetTouchpadMove(ControllerState::Axis& from, ControllerState::Axis& to) const {
        if (cur_state_.OnTouchpad && touchpad_frames_>0) {
            ...
            BL_LOG("****** GetTouchpadMove: ******\n");
            BL_LOG("Axis: \n");
            BL_LOG("<from> Touchpad: %.2f, %.2f\n", from.x, from.y);
            BL_LOG("<to>   Touchpad: %.2f, %.2f\n", to.x, to.y);
            BL_LOG("*********************************\n");
        }
        return 0;
    }
    ...
    virtual bool SetCommand(TRACKED_DEVICE_COMMAND cmd, uint32 param1, uint32 param2) const = 0;
...
public:
    ...
    virtual bool GetCameraFrameSize(uint32& width, uint32& height) = 0;
    virtual bool GetCameraIntrinsic(float& focalLength_x, float& focalLength_y, float& center_x, float& center_y) = 0;
    virtual bool GetCameraToHMDTransform(math::Matrix3& trans) = 0;
    virtual bool GetCameraProjection(math::Matrix4 & proj) = 0;

    virtual graphics::ITexture* GetTexture() = 0;
    virtual bool StartVideoPreview() = 0;
    virtual bool UpdateFrame() = 0;
    virtual void StopVideoPreview() = 0;
};
...
public:
    ...
    // Yu-Mei
    virtual HMDCamera* GetHMDCamera() = 0;
    void SetRenderSize(int width, int height) {
        renderWidth_ = width;
        renderHeight_ = height;
        return;
    }
    void SetProjMatrix(math::Matrix4 proj, HMD_EYE eye=HMD_EYE_NEUTRAL) {
        if (HMD_EYE_NEUTRAL == eye) {
            matProjLeft_ = matProjRight_ = proj;
        } else if (HMD_EYE_RIGHT == eye) {
            matProjRight_ = proj;
        } else { // HMD_EYE_RIGHT == eye
            matProjLeft_ = proj;
        }
        return;
    }
    void SetHMDPose(math::Matrix3 pose, HMD_EYE eye=HMD_EYE_NEUTRAL) {
        BL_MUTEX_LOCK(mutex_);
        if (HMD_EYE_NEUTRAL==eye) {
            matHMDTrackingPose_ = pose;
            matLeftEyePoseOffset_ = matRightEyePoseOffset_ = math::Matrix3::Identity;
        }
        else if (HMD_EYE_LEFT==eye) {
            matLeftEyePoseOffset_ = pose;
            matHMDTrackingPose_ = math::Matrix3::Identity;
        }
        else { // HMD_EYE_RIGHT==eye
            matRightEyePoseOffset_ = pose;
            matHMDTrackingPose_ = math::Matrix3::Identity;
        }
        return;
    }
